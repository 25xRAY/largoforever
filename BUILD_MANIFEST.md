...
- **Graduation ceremony (post-ship):** `constants.ts` — May 18, 2026, 10:30 a.m., The Show Place Arena, Upper Marlboro, MD (`GRADUATION_DATE_ISO`, `GRADUATION_CEREMONY_SUMMARY`); homepage/dashboard countdowns, `DeadlinesWidget`, FAQ schema (`seo.ts`), help FAQ, Ed RonIQ system prompt.
- **Homepage & floating Ed RonIQ (post-ship):** `(public)/page.tsx` Fortune 50–style hero + milestones + feature grid + Wall of Wins preview + Ed RonIQ section + L.I.O.N. + support team + CTA; crest `Image` `/images/largo-crest-red.png`; `CrisisBanner alwaysExpanded`. `components/ai/EdRoniqFloat.tsx` — `POST /api/ai/chat` with `message` + `conversationHistory`, reads `reply`; wired in `(public)/layout.tsx` and `DashboardLayout` (student + admin dashboard); hidden on `/dashboard/ed-roniq`, `/admin`, auth/onboarding paths.
