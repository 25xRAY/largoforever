...
export function WallOfWinsPageClient() {
  ...
  useEffect(() => {
    if (!stats) return;
    animateValue(0, stats.totalDollars, 1200, (n) =>
      setStatsDisplay((s) => ({ ...s, totalDollars: n }))
    );
    animateValue(0, stats.acceptanceCount, 1200, (n) =>
      setStatsDisplay((s) => ({ ...s, acceptanceCount: n }))
    );
    animateValue(0, stats.fullRideCount, 1200, (n) =>
      setStatsDisplay((s) => ({ ...s, fullRideCount: n }))
    );
    animateValue(0, stats.militaryCount + stats.tradeCount, 1200, (n) =>
      setStatsDisplay((s) => ({ ...s, militaryCount: n }))
    );
  }, [stats]);
    ...
    <div className="space-y-8">
      <section className="rounded-card bg-gradient-to-br from-navy-600 to-navy-800 p-8 text-white">
        ...
        <p className="mt-2 text-white/90">
          Scholarships, acceptances, and milestones from the Class of 2026.
        </p>
        <div className="mt-8 grid grid-cols-2 gap-6 lg:grid-cols-4">
          <div>
            <p className="text-2xl font-bold text-gold-500 lg:text-3xl">
              {formatDollars(statsDisplay.totalDollars)}
            </p>
            <p className="text-sm text-white/80">Total scholarships</p>
          </div>
          <div>
            <p className="text-2xl font-bold text-gold-500 lg:text-3xl">
              {statsDisplay.acceptanceCount}+
            </p>
            <p className="text-sm text-white/80">College acceptances</p>
          </div>
          <div>
            <p className="text-2xl font-bold text-gold-500 lg:text-3xl">
              {statsDisplay.fullRideCount}
            </p>
            <p className="text-sm text-white/80">Full-ride scholarships</p>
          </div>
          <div>
            <p className="text-2xl font-bold text-gold-500 lg:text-3xl">
              {statsDisplay.militaryCount}
            </p>
            ...
            {wins.map(
              (w: {
                ...
                <WinCard
                  ...
                  id={w.id}
                  type={
                    w.type as "SCHOLARSHIP" | "ACCEPTANCE" | "MILITARY" | "JOB" | "AWARD" | "OTHER"
                  }
                  title={w.title}
                  ...
                  studentName={
                    w.user ? [w.user.firstName, w.user.lastName].filter(Boolean).join(" ") : null
                  }
                />
              )
            )}
